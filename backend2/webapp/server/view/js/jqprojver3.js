$.fn.shop = function(options, cartSelector) {
    ...
    var cart = (function(selector) {
        ...
        function createTable(selector) {
            ...
            createRow("#" + selector, 5, '<td rowspan="2" colspan="5">', ['<label id="labelform" for="submit-form" tabindex="0"><img src="img/checkout-logo-large.png" alt="submit"></label>'], '<tr id="' + selector + '-paypal">');
        }
        ...
        function addItem(sku, name, price) {
            ...
            if (sku.beforeproductlist) {
                var beforeproductlist = sku.beforeproductlist;
            }
            ...
            if ($("#" + sku).length) {
                ///////////////////////server side///////////////////////////
                if (!beforeproductlist) {
                    socket.emit('addtocart', { sku: sku }, false);
                }
                //////////////////////////////////////////////////
                ...
                ////////////////////////socket add item//////////////////
                if (!beforeproductlist) {
                    socket.emit('addtocart', { sku: sku }, true);
                }
                /////////////////////////////////////////////////////////
                ...
                $("#" + sku).find("td:nth-child(1)").click(function() {
                    ...
                    //////////////////////remove emit////////////////////
                    var socket = io.connect('http://localhost:3000');
                    socket.emit('delete', { sku: sku });
                });
                totalChange("#" + selector + "-end");
                $("#" + sku).change(function() {
                    var quantity = AddToCreatedRow($("#" + sku), false, selector);
                    socket.emit('addtocart', { sku: sku }, false, quantity);
                });
            }
        ...
        function totalChange(total) {
            ...
            $(total + " td").each(function(index, value) {
                if (index == 2) {
                    $(value).text(calcTotal(totalObj.count))
                }
                // $(value).text(totalObj.price[index]);
                if (index == 4) {
                    $(value).text("$" + calcTotalWithPrice(totalObj.count, totalObj.price))
                }
            });
    ...
    for (var i = 0; i < products.length; ++i) {
        var prod = products[i];
        this.product = new ProductList(prod.items, that, cart, cartSelector);
    }
    ///////////////////////////////////
